...
class BaseHypergraph:
    ...
    # def remove_node_edge_association(self, node_idx, edge_idx, group_name='main'):
    #     """
    #     Remove the association between a node and a hyperedge in the hypergraph.
    #
    #     Args:
    #         node_idx (int): The index of the node to be removed from the hyperedge.
    #         edge_idx (int): The index of the hyperedge from which the node will be removed.
    #         group_name (str, optional): The name of the group containing the hyperedge. Defaults to 'main'.
    #     """
    #     # Fetch the H matrix for the specified group
    #     H = self._fetch_H_of_group("v2e", group_name)
    #
    #     # Get the indices of the node and hyperedge in the H matrix
    #     node_idx_in_H = torch.where(H.indices() == node_idx)[0]
    #     edge_idx_in_H = torch.where(H.indices() == edge_idx)[1]
    #
    #     # Check if the node is associated with the hyperedge
    #     if node_idx_in_H.size(0) > 0 and edge_idx_in_H.size(0) > 0:
    #         # Remove the association by setting the corresponding element in H to 0
    #         H[node_idx_in_H, edge_idx_in_H] = torch.tensor(0.0, dtype=torch.float32, device=self.device)
    #
    #         # Update the H matrix in the cache
    #         self.cache["H"] = H
    #
    #     else:
    #         print(f"Node {node_idx} is not associated with hyperedge {edge_idx} in group {group_name}.")
    ...
    def _add_hyperedge(
        ...
        r"""Add a hyperedge to the specified hyperedge group.
            _add_hyperedge 方法是用于向超图中的特定超边组添加超边的
        Args:
    ...
    def remove_hyperedges(
        self,
        e_list_v2e: Union[List[int], List[List[int]]],
            # e_list_v2e：一个包含超边的列表，描述顶点如何指向超边。
        e_list_e2v: Union[List[int], List[List[int]]],
            # e_list_e2v：一个包含超边的列表，描述超边如何指向顶点。
        group_name: Optional[str] = None,
            # 指定要从中移除超边的超边组。如果未指定，则从所有超边组中移除超边。
    ):
        r"""Remove the specified hyperedges from the hypergraph.
            remove_hyperedges 方法用于从超图中移除指定的超边
        Args:
            ``e_list_v2e`` (``Union[List[int], List[List[int]]]``): A list of hyperedges describes how the vertices point to the hyperedges.
            ``e_list_e2v`` (``Union[List[int], List[List[int]]]``): A list of hyperedges describes how the hyperedges point to the vertices.
